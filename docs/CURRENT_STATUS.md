...
## 1. 현재 학습 진행 단계
* **진행 단계**: **7단계 완료 (8단계 시작 대기)**
* **목표 진도**: [DEVELOPMENT_PLAN.md](file:///c:/Users/Dalyume/Documents/DAL/Coding/notion-clone/docs/DEVELOPMENT_PLAN.md) 기준 8단계 시작
...
| **7단계** | 문서 삭제 API | **완료 (Done)** | DELETE 요청 및 상세/선택 해제 대응 |
...
* [x] **7단계 문서 삭제 API**: DELETE 요청, 중복 요청 방지, 목록·선택·상세 상태 초기화 구현.

---

## 3. 진행 중인 기능
* **현재 태스크**: 7단계 문서 삭제 완료
  * 삭제 버튼, DELETE API와 실패 상태 구현 완료
  * 목록·선택·상세 초기화 및 Mock `200 OK` 응답 검증 완료

---

## 4. 다음 구현 예정 기능
* **대상**: 8단계 - 디바운스 자동 저장
  * 입력 변경 여부와 자동 저장 조건 정의
  * 1.5초 저장 타이머와 Cleanup 구현
  * 저장 중 문서 전환과 수동 저장 충돌 방지
...
* `DELETE /api/pages/{pageId}` - 선택 문서 삭제
