...
- [x] Network 탭과 React DevTools로 흐름 확인
- [x] 린트 및 프로덕션 빌드 통과
- [x] `CURRENT_STATUS.md`와 학습 체크리스트 갱신
...
- [x] API 계약의 `POST /api/pages` 확인
- [x] `pageApi.ts`에 `createPage` 함수 작성
- [x] 사이드바에 새 페이지 버튼 배치
- [x] 생성 API를 클릭 이벤트 핸들러에서 호출
- [x] 생성 중 중복 클릭 방지 상태 처리
- [x] 성공 후 목록 갱신
- [x] 생성된 문서 ID를 선택 상태에 반영
- [x] 실패 안내 UI 처리
- [x] Network 탭에서 `201 Created` 응답 확인
- [x] 린트 및 프로덕션 빌드 통과
- [x] `CURRENT_STATUS.md`와 학습 체크리스트 갱신
...
- [x] API 계약의 `PATCH /api/pages/{pageId}` 확인
- [x] 수정 요청 타입 정의
- [x] `pageApi.ts`에 `updatePage` 함수 작성
- [x] 제목과 본문을 제어 컴포넌트로 연결
- [x] 선택 문서 변경 시 로컬 입력값 동기화
- [x] 저장 버튼과 이벤트 핸들러 연결
- [x] 저장 중·성공·실패 상태 표시
- [x] 저장 성공 후 사이드바 제목 갱신
- [x] Network 탭에서 요청 Body와 응답 확인
- [x] 린트 및 프로덕션 빌드 통과
- [x] `CURRENT_STATUS.md`와 학습 체크리스트 갱신
